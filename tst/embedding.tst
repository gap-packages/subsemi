...
gap> ForAll(ConjugacyClassRepSubsemigroups(T3,S3), x-> EmbedAbstractSemigroup(Rows(MulTab(Semigroup(x))), Rows(MulTab(T3))) <> fail);
true
gap> TL3 := JonesMonoid(3);;mt := MulTab(TL3);;
gap> ForAll(AllSubsemigroups(TL3), x-> EmbedAbstractSemigroup(Rows(MulTab(Semigroup(x))), Rows(mt)) <> fail);
true
